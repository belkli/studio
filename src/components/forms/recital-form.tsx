...
const RepertoireItem = ({ index, remove, fields }: { index: number; remove: (index: number) => void; fields: FieldArrayWithId<any, 'repertoire', 'id'>[] }) => {
...
export function RecitalForm({ user, student, onSubmit, initialData, isEditing = false, onCancel }: RecitalFormProps) {
    ...
    const { guideline, isDurationOutsideGuidelines, isDurationUnder } = useMemo(() => {
        ...
        const guidelineForGrade = (durationGuidelines as Record<string, { min: number; max: number; label: string }>)[grade];
